//PixelBoardBase.h
//The BASE class for all modules of PixelBoardBase
//Provide function to mapping coordinates to PixelBoardBase's ZigZag coordinate
#ifndef _PixelBoardBase_h
#define _PixelBoardBase_h
...
class PixelBoardBase{
public:
  PixelBoardBase(){};
  PixelBoardBase(Adafruit_NeoPixel* s) : stripPtr(s){ };
  ~PixelBoardBase(){ stripPtr = NULL; };
  virtual void update(unsigned long currentMillis)=0;
  virtual void reset() = 0;
  unsigned long lastupdate = 0;

  //Function to get index on the strip;
  byte getPixelIndex(byte x){
	if( (x / BOARDWIDTH) % 2 == 0){
      return x;
	}else{
      return (( (x / BOARDWIDTH) * BOARDWIDTH ) + BOARDWIDTH - 1)  - (x % BOARDWIDTH);
	}
  }
